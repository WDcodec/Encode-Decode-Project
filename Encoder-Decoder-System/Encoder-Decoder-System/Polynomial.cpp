...
namespace galois
{
		...
		inline Field_Element& Polynomial::operator[] (const std::size_t& term) {
			assert(term < poly_.size());
			return poly_[term];
		}

		inline const Field_Element& Polynomial::operator[](const std::size_t& term) const {
			assert(term < poly_.size());
			return poly_[term];
		}
...
//		// Function that check whether a plynomial is monic or not.
		inline bool Polynomial::monic() const {
...
//		// Function that claculate the derivative of polynomials
		inline Polynomial Polynomial::derivative() const {
